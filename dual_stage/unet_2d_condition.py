...
class UNet2DConditionModel(
    ...
    def DoG(self, sample, ksize=5, sigma1=1.0, sigma2=2.0):
        g1 = K.gaussian_blur2d(sample, (ksize, ksize), (sigma1, sigma1))
        g2 = K.gaussian_blur2d(sample, (ksize, ksize), (sigma2, sigma2))
        return g1 - g2
    
    def apply_DoG_2_up(self, sample, filter_factor):
        DoG_result = self.DoG(sample)  # still tensor on GPU
        sample = sample + filter_factor * DoG_result  # residual enhancement
        return sample
    #####################
    # applg Gobar
    def apply_gabor_2_up(self, sample, filter_factor):
    ...
    def forward(
        ...
        hidden_mask: Dict[str, List[int]] = None,
        replace_mask: Union[List[Dict[str, List[int]]], float] = 0,
        replace_scale: Union[List[Dict[str, List[int]]], float] = 1.0,
        ...
        downs = []
        for di, downsample_block in enumerate(self.down_blocks):
            ...
            # print("res_samples before", res_samples[-1][0,0,0,:5])
            downs.append(sample.detach().cpu().numpy())

            if isinstance(replace_mask, dict): replace_mask_ = replace_mask['down'][di]
            elif isinstance(replace_mask, float): replace_mask_ = replace_mask
            ...
            if hidden_mask != None:
                if isinstance(replace_on, dict) or  (isinstance(replace_on, str) and replace_on == 'mean'):
                    if (isinstance(replace_on, str) and replace_on == 'mean'):
                        ...
                        replace_on_ = torch.tensor(replace_on['down'][di]).to(sample.device)
                    if hidden_mask['down'][di] != []:
                    ...
                    if hidden_mask['down'][di] != []:
                        if isinstance(replace_mask_, float):
                            sample[:, hidden_mask['down'][di]] *= replace_mask_
                        else:
                            sample[:, hidden_mask['down'][di]] = torch.tensor(replace_mask_).type(torch.HalfTensor).to(sample.device)*replace_scale#*sample.max()
                        

                # print('after', sample[:, hidden_mask['down'][di]])
            # print("res_samples after", res_samples[-1][0,0,0,:5])
        ...
        if self.mid_block is not None:
            ...
            if (
                ...
                sample += down_intrablock_additional_residuals.pop(0)
            mids.append(sample.detach().cpu().numpy())

            if isinstance(replace_mask, dict): replace_mask_ = replace_mask['mid'][0]
            elif isinstance(replace_mask, float): replace_mask_ = replace_mask
            else: assert("unknown type of replace_mask")
            ...
            if hidden_mask != None:
                if isinstance(replace_on, dict) or  (isinstance(replace_on, str) and replace_on == 'mean'):
                    if (isinstance(replace_on, str) and replace_on == 'mean'):
                        ...
                        replace_on_ = torch.tensor(replace_on['mid'][0]).to(sample.device)
                    if hidden_mask['mid'][0] != []:
                        sample[:, hidden_mask['mid'][0]] += sample[:, hidden_mask['mid'][0]] * replace_mask_ * replace_on_
                    
                elif (isinstance(replace_on, str) and replace_on == 'freq') and replace_mask_!=1.0:
                    print("run fft on mid")
                    cutoff_freq_ = cutoff_freq if isinstance(cutoff_freq, float) else cutoff_freq['mid'][0]
                    sample = self.remove_noise_with_fourier(sample, cutoff_freq_, replace_mask_)
                    
                elif replace_on == None:
                    if hidden_mask['mid'][0] != []:
                        if isinstance(replace_mask_, float):
                            sample[:, hidden_mask['mid'][0]] *= replace_mask_
                        else:
                            sample[:, hidden_mask['mid'][0]] = torch.tensor(replace_mask_).type(torch.HalfTensor).to(sample.device)*replace_scale#*sample.max()
                        


                # print('mid', sample[:, hidden_mask['mid']])
            
        
        if is_controlnet:
            sample = sample + mid_block_additional_residual
        # print("mid[0]", sample[0][0][0][:5])

        # for rs in range(len(down_block_res_samples)):
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            # for rs in range(len(res_samples)):
            #     print("res_samples[{}]".format(rs), res_samples[-(rs+1)][0][0][0][:5])
            ...
            # print("up{}[0]".format(i), sample[0][0][0][:5])
            ups.append(sample.detach().cpu().numpy())

            if isinstance(replace_mask, dict): replace_mask_ = replace_mask['up'][i]
            elif isinstance(replace_mask, float): replace_mask_ = replace_mask
            else: assert("unknown type of replace_mask")
            ...
            if hidden_mask != None:
                ...
                if isinstance(replace_on, dict) or  (isinstance(replace_on, str) and replace_on == 'mean'):
                    ...
                    if (isinstance(replace_on, str) and replace_on == 'mean'):
                        ...
                        replace_on_ = torch.tensor(replace_on['up'][i]).to(sample.device)
                    if hidden_mask['up'][i] != []:
                        sample[:, hidden_mask['up'][i]] += sample[:, hidden_mask['up'][i]] * replace_mask_ * replace_on_
                    
                elif (isinstance(replace_on, str) and replace_on == 'freq') and replace_mask_!=1.0:
                    print("run fft on up")
                    cutoff_freq_ = cutoff_freq if isinstance(cutoff_freq, float) else cutoff_freq['up'][i]
                    sample = self.remove_noise_with_fourier(sample, cutoff_freq_, replace_mask_)
                    ...
                    if hidden_mask['up'][i] != []:
                        if isinstance(replace_mask_, float):
                            ...
                            sample[:, hidden_mask['up'][i]] = torch.tensor(replace_mask_).type(torch.HalfTensor).to(sample.device)*replace_scale#*sample.max()
                
                ## apply filter to up block
                if apply_filter:
                    print("run gabor on up")
                    sample = self.apply_gabor_2_up(sample, filter_factor)
                        
        # print("up[2][0]", sample[0][0][0][:5])

                # print("up", sample[:, hidden_mask['up'][i]])
        # 6. post-process
        ...
        if not return_dict:
            ...
            returns = (UNet2DConditionOutput(sample=sample),)

        if return_hidden:
            returns += ({'down': downs, 'mid':mids, 'up': ups},)
